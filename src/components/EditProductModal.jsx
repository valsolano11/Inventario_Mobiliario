...
const EditProductModal = ({ isOpen, onClose, product }) => {
  ...
  const fetchProductDetails = async (productId) => {
    ...
    try {
      ...
      if (response.status === 200) {
        const { nombre,volumen,marca,descripcion, UnidadMedidaId,SubcategoriaId,UsuarioId, EstadoId,} = response.data;
        setFormData({
          nombre: nombre || "",
          volumen: volumen || "",
          marca: marca || "",
  ...
  const handleUpdate = async () => {
    const { nombre,volumen,marca,descripcion, UnidadMedidaId,SubcategoriaId,UsuarioId, EstadoId,}= formData;

    if (!nombre || !volumen || !marca || !descripcion || !UnidadMedidaId || !SubcategoriaId || !UsuarioId || !EstadoId) {
